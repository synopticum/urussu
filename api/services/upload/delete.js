...
const { hashImage } = require("../../db/helpers");

module.exports = async function (fastify, opts) {
  fastify.register(() => registerRoutes(fastify, opts));
};
...
async function registerRoutes(fastify, opts) {
  fastify.route({
    ...
    handler: async (request, reply) =>
      await verifyVkAuth(request, reply, remove),
  });
}
...
async function remove(request, reply) {
  try {
    ...
    reply.code(200).send({
      id: hashImage(year, key),
      year: year.includes("_") ? year.split("_")[1] : year,
      url: key,
    });
  } catch (e) {
...
async function removePhotoFromModel(type, id, year) {
  ...
  const hasImages = Object.keys(model._doc.images).length;

  await Model.findOneAndUpdate(
    query,
    { images: hasImages ? model._doc.images : null },
