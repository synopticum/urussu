...
async function canRemove(request, dotId) {
  ...
  if (userId) {
    const dotModel = await DotModel.findOne({
      id: { $regex: dotId, $options: "i" },
    });
    const dotAuthorId = dotModel._doc ? dotModel._doc.authorId : undefined;
...
async function removePhotos(request, reply, dotId) {
  try {
    const dot = await DotModel.findOne({
      id: { $regex: dotId, $options: "i" },
    });
...
async function removeComments(request, reply, dotId) {
  try {
    await CommentModel.deleteOne({
      originId: { $regex: dotId, $options: "i" },
    });
  } catch (e) {
    reply.type("application/json").code(500);
    return {
      error: `Unable to remove a dot: error when deleting dot comments`,
    };
  }
}
